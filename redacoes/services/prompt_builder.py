...
def validar_resposta_correcao(resposta: str, redacao_original: str) -> dict[str, Any]:
    ...
    if not isinstance(redacao_original, str) or not redacao_original.strip():
        raise PromptResponseValidationError("A redação original deve ser texto não vazio.")
    if not isinstance(resposta, str) or not resposta.strip():
        raise PromptResponseValidationError("A resposta está vazia.")
    if len(resposta) > 200_000:
        raise PromptResponseValidationError("A resposta excede o limite aceito.")

    texto = resposta.strip()
    if texto.startswith("```") or texto.endswith("```"):
        raise PromptResponseValidationError("A resposta não pode conter Markdown.")

    try:
        dados = json.loads(texto)
    except json.JSONDecodeError as erro:
        raise PromptResponseValidationError("A resposta não é um JSON válido.") from erro
    ...
    if not isinstance(competencias, list) or len(competencias) != 5:
        raise PromptResponseValidationError("Devem existir exatamente cinco competências.")
    ...
    if dados["avaliacao_possivel"]:
        ...
        if any(not _nota_valida(nota) for nota in notas):
            raise PromptResponseValidationError("Uma ou mais notas são inválidas.")
        if (
            not isinstance(dados["nota_total"], int)
            or isinstance(dados["nota_total"], bool)
            or dados["nota_total"] != sum(notas)
        ):
            raise PromptResponseValidationError("nota_total não corresponde à soma.")
    else:
...
def _exigir_chaves(objeto: dict, esperadas: set[str], contexto: str) -> None:
    if set(objeto) != esperadas:
        raise PromptResponseValidationError(f"Chaves inválidas em {contexto}.")
...
def _validar_competencia(competencia: Any, numero: int, redacao: str) -> None:
    ...
    if competencia["numero"] != numero:
        raise PromptResponseValidationError("Competências fora de ordem.")
    if competencia["nome"] != NOMES_COMPETENCIAS[numero]:
        raise PromptResponseValidationError(f"Nome inválido na competência {numero}.")
    if competencia["nota"] is not None and not _nota_valida(competencia["nota"]):
        raise PromptResponseValidationError(f"Nota inválida na competência {numero}.")
    _exigir_string(competencia["justificativa"], "justificativa")
    _validar_evidencias(competencia["evidencias"], redacao)
    _validar_lista_strings(competencia["pontos_fortes"], "pontos_fortes")
    _validar_lista_strings(competencia["melhorias"], "melhorias")


def _validar_evidencias(evidencias: Any, redacao: str) -> None:
...
def _validar_intervencao(intervencao: Any, redacao: str) -> None:
    ...
    _validar_evidencias(intervencao["evidencias"], redacao)
